...
export default {
  ...
  methods : {
    ...
    addContact () {
        let { name, tel, address} = this
        let param = { name, tel, address }

        axios
        .post(`${BASEURL}/contacts`, param)
        .then((response) => {  
            this.result = response.data
            this.no = result.no
        })
    ...
    changePhoto () {
        ...
        axios.post(`${BASEURL}/contacts/${this.no}/photo`, data)
        .then((response) => {
            this.result = response.data
        })
    }
